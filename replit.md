...
*   **AI Futures Trader:**
    ...
    *   **Pattern Detection (Live Scanner):** Detects 11 live patterns: 3 Bar Play, Buy/Sell Setup, Pivot Breakout, Climax/Exhaustion Reversal, Wedge Breakout (falling/rising), Cup & Handle (bullish), Inverse Cup & Handle (bearish), Double Bottom (bullish), Double Top (bearish), Head & Shoulders (bearish), and Inverse Head & Shoulders (bullish). All use local peak/trough detection, linear regression, neckline slope projection, volume surge confirmation (>1.5x avg), and 11-factor confluence scoring. Backtester additionally supports Retest and 4 Bar Play (13 total). All patterns have individual toggles.
